...
describe CoffeeAPI::CoffeeMenu do
  describe '#menu_items' do
    it 'returns a list of at least four coffees' do
      expect(subject.coffees.length).to be >= 4
    end

    it 'has names for each of the coffees' do
      expect(subject.coffees).to each_have_key(:name)
    end

    it 'has prices for each of the coffees' do
      expect(subject.coffees).to each_have_key(:price)
    end

    it 'has caffeine levels for each of the coffees' do
      expect(subject.coffees).to each_have_key(:caffeine_level)
    end

    it 'has milk ratios for each of the coffees' do
      expect(subject.coffees).to each_have_key(:milk_ratio)
    end
  end
end
